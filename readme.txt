...
The cronproj framework solves this problem by automating the process of deploying infrascture on aws for 
code that is written in python. Once you have the python code written and functioning, 
all you need to do is passin your root credentials into the "root.json" file and create a template for each script and define your cron scedules.

the framework is broken down into two parts. base infrascture(the cronproj folder) and then script infrascture (this part is variable).
...
|---> ecr\ ---> ecr_policy.json
|---> ecr\ ---> ecr.json
|---> iam\ ---> inline_policies\ ---> ecr_resouce_access.json
...
|---> script_infasctructure\ ---> test-scriptt ---> image ---> image.json
|---> script_infasctructure\ ---> test-scriptt ---> lambda ---> lambda.json
|---> script_infasctructure\ ---> test-scriptt ---> s3 ---> bucket.json
...
step six: create a secruity group that only allows ssh traffic.
...
next create the name of the role, and in the role_policies.json file, create a dictonary with the ARN of all the policies that you want attached to that role.
next ceate the name of the instance profile and the code will attach the role and its policies to the new 
instance profile.
...
the code bucket is used for whenever a new script is added all of the neccessary file get sent to this bucket under the prefix of the name of script. this enables cron schedules to be automated.
...
the code then also pushes the python_file varible to s3.
The python_file is the code that get excuted on the home ec2_instance. The code creates a temporary ec2_instance with 
a user data script that pulls the image out of the ecr and runs the container.


lambda.py:

the final peice is the lambda function. The lambda function terminates the instance the python_file code creates.
